...
const Header: React.FC<HeaderProps> = ({ backgroundColor }) => {
  ...
  return (
    <header
      className={`flex items-center justify-between px-4 py-4 md:py-6 md:px-8 fixed w-full z-50 font-alliance`}
      style={navbarStyle}
    >
   ...
   md:space-x-6 md:px-6 md:bg-[#FFFFFF]/12 md:rounded-full md:py-3 md:text-xs font-semibold md:text-[#FFFFFF] md:border-2 md:border-[#FFFFFF]/20 backdrop-blur-sm ${
     isOpen ? "translate-x-0" : "translate-x-full"
      ...
      <Link href="/">
        <button
          className={`${inter.className} px-2 py-3 md:px-6 text-xs bg-transparent rounded-full border-2 border-[#FFFFFF] text-[#FFFFFF] font-medium cursor-pointer hover:bg-[#FFFFFF]/20 transition-colors duration-300`}
        >
          Get Started Now
        </button>
      </Link>
